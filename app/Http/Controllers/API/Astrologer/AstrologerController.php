...
class AstrologerController extends Controller
{
    ...
    public function addAstrologer(Request $req)
    {
        try {
            ...
            //if otpless login then password by env file
            if(empty($req->password)){
                $req->password = Hash::make(env('ASTROLOGER_DEFAULT_PASSWORD'));
            }
    ...
    public function loginAstrologer(Request $req)
    {
        try {
            ...
            //default password from env for the otp less login
            if($credentials && empty($credentials['password'])){
                $credentials['password'] = env('ASTROLOGER_DEFAULT_PASSWORD');
            }

            if($email_credentials && empty($email_credentials['password'])){
                $email_credentials['password'] = env('ASTROLOGER_DEFAULT_PASSWORD');
            }
